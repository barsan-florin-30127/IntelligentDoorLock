...
public class DoorLockController implements ControllerInterface {
    ...
    public WriteAccessLog writeAccessLog = new WriteAccessLog();
    ...
    public DoorStatus enterPin(String pin) throws TooManyAttemptsException, InvalidPinException {
        ...
        if (accessKeyTemporary.equals(new AccessKey(MASTER_KEY))) {//tratam cazul in care este introdus master_key-ul
            ...
            if (this.door.getStatus() == DoorStatus.CLOSE) {
                ...
                writeAccessLog.writeDataLog(new AccessLog(this.MASTER_TENANT_NAME, LocalDateTime.now(), "Enter Pin", this.door.getStatus(), "Correct Pin"));
            } else {
                this.door.lockDoor();
            ...
            if (numberOfIncorrectIntroducedPin == 2) {//if ce trateaza cazul in care este introdus un pin gresit de mai multe ori
                ...
                writeAccessLog.writeDataLog(new AccessLog("A tenant ", LocalDateTime.now(), "Enter pin", this.door.getStatus(), "Too many incorrect pins entered"));
                ...
                if (flag == 1) {//tratam cazul in care este introdus un pin regasit in lista cu accessKey-uri
                    if (this.door.getStatus() == DoorStatus.CLOSE) {
                        ...
                        writeAccessLog.writeDataLog(new AccessLog(tenantList.get(tenantPinIndex).getName(), LocalDateTime.now(), "Enter Pin", this.door.getStatus(), "Correct Pin"));
                    } else {
                        this.door.lockDoor();
                        System.out.println("Door was locked");
                        this.accessLogs.add(new AccessLog(tenantList.get(tenantPinIndex).getName(), LocalDateTime.now(), "Enter Pin", this.door.getStatus(), "Correct Pin"));
                    ...
                    writeAccessLog.writeDataLog(new AccessLog("A tenant ", LocalDateTime.now(), "Enter pin", this.door.getStatus(), "Incorrect Pin"));
    ...
    public void addTenant(String pin, String name) throws TenantAlreadyExistsException {
        ...
        if (flag == 1) {
            ...
            writeAccessLog.writeDataLog(new AccessLog(temporaryTenant.getName(), LocalDateTime.now(), "Add Tenant", this.door.getStatus(), "Tenant Already Exists"));
            ...
            writeAccessLog.writeDataLog(new AccessLog(temporaryTenant.getName(), LocalDateTime.now(), "Add Tenant", this.door.getStatus(), "Tenant has been added"));
        }
    }
    ...
    public void removeTenant(String name) throws TenantNotFoundException {
        ...
        if (flag == 1) {
            ...
            writeAccessLog.writeDataLog(new AccessLog(temporaryTenant.getName(), LocalDateTime.now(), "Remove Tenant", this.door.getStatus(), "Tenant has been removed"));
            ...
            writeAccessLog.writeDataLog(new AccessLog(temporaryTenant.getName(), LocalDateTime.now(), "Remove Tenant", this.door.getStatus(), "Tenant Not Found"));
